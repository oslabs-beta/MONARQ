...
const { queryMap, generateQuery, typeChecker, typeTrim, grabFields, buildString, grabArgs, argsStrFormatter, varStrBuild } = queryGen;
...
describe('generateQuery function', () => {
  it('handles single scalar arguments', () => {
    expect(generateQuery(schema, 'book', scalarTypes)).toBe('query ( $id: ID!, ) { book ( id:$id, ) { id name author { id name publishers { id name createdAt updatedAt } createdAt updatedAt } createdAt updatedAt } }');
  });

  it('handles multiple scalar arguments', () => {
    expect(generateQuery(schema, 'books', scalarTypes)).toBe('query ( $pageSize: Int!, $page: Int!, ) { books ( pageSize:$pageSize, page:$page, ) { info { count pages next prev } results { id name author { id name publishers { id name createdAt updatedAt } createdAt updatedAt } createdAt updatedAt } } }');
  });
...
describe('grabFields function', () => {
  it('handles types with all scalar fields', () => {
    ...
    expect(grabFields(schema, customTypeName, schema.getType(customTypeName).getFields(), recursiveBreak, scalarTypes)).toEqual({
      name: '',
      author: ''
    });
  });
...
describe('arguments string formatting', () => {
  it('handles single scalar arguments', () => {
    ...
    expect(argsStrFormatter(buildString(grabArgs(schema, argsArr, scalarTypes)[0]))).toBe('id:$id,');
  });

  it('handles multiple scalar arguments', () => {
    const argsArr = schema.getQueryType().getFields().books.args;
    expect(argsStrFormatter(buildString(grabArgs(schema, argsArr, scalarTypes)[0]))).toBe('pageSize:$pageSize, page:$page,');
  });

  it('handles arguments of custom types', () => {
    const argsArr = schema.getMutationType().getFields().updateBook.args;
    expect(argsStrFormatter(buildString(grabArgs(schema, argsArr, scalarTypes)[0]))).toBe('id:$id, book : { name:$name, }');
  });
})
...
describe('variables string formatting', () => {
  it('handles single scalar arguments', () => {
    const argsArr = schema.getQueryType().getFields().book.args;
    expect(varStrBuild(grabArgs(schema, argsArr, scalarTypes)[1])).toBe('$id: ID!,');
  });

  it('handles multiple scalar arguments', () => {
    const argsArr = schema.getQueryType().getFields().books.args;
    expect(varStrBuild(grabArgs(schema, argsArr, scalarTypes)[1])).toBe('$pageSize: Int!, $page: Int!,');
  });

  it('handles arguments of custom types', () => {
    const argsArr = schema.getMutationType().getFields().updateBook.args;
    expect(varStrBuild(grabArgs(schema, argsArr, scalarTypes)[1])).toBe('$id: ID!, $name: String,');
  });
})
